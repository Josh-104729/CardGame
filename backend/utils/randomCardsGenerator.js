...
const RandomCardsGenerator = (members, initialCnt, total) => {
    const randomKey = 20010103;
    const randomArea = 54;
    let players = new Array();
    let rest_cards = new Array();


    let card_pack = new Array();
    ...
    for (let i = 0; cnt < total; i++) {
        let tmp = Math.floor(Math.random() * randomKey) % randomArea;
        if (!hash_arr[tmp]) {
    ...
    for (let i = 0; i < members; i++) {
        players[i].sort((a, b) => a.number - b.number ? a.number - b.number : a.type - b.type);
    }
    return { players, rest_cards };
};

module.exports = { RandomCardsGenerator };